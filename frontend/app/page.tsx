...
export default function LoginPage() {
  ...
  const handleMfa = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post('/auth/login/mfa', { username, password, token })
      saveAuth(res.data.access_token, res.data.user)
      router.push('/dashboard')
    } catch (err: any) {
      setError(err.response?.data?.message || 'Token inválido')
    } finally {
      setLoading(false)
    }
  }
          ...
          {step === 'credentials' ? (
            <form onSubmit={handleCredentials} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  required
                  autoComplete="off"
                  className="w-full bg-black/40 border border-[rgba(0,255,100,0.15)] px-4 py-3 text-white font-[family-name:var(--font-mono)] text-sm tracking-wider outline-none focus:border-[rgba(0,255,100,0.5)] transition-all placeholder:text-white/10"
                />
              </div>
            ...
            <form onSubmit={handleMfa} className="space-y-5">
              ...
              <div className="text-center mb-6">
                <p className="font-[family-name:var(--font-mono)] text-[10px] text-[rgba(0,255,100,0.5)] tracking-[2px] uppercase">
                  Autenticação em dois fatores
                </p>
                <p className="font-[family-name:var(--font-mono)] text-[11px] text-white/40 mt-2">
                  Digite o token do seu autenticador
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  autoComplete="off"
                  maxLength={6}
                  className="w-full bg-black/40 border border-[rgba(0,255,100,0.15)] px-4 py-3 text-white font-[family-name:var(--font-mono)] text-2xl tracking-[12px] text-center outline-none focus:border-[rgba(0,255,100,0.5)] transition-all placeholder:text-white/10"
                />
              </div>
              ...
              <button
                type="button"
                onClick={() => { setStep('credentials'); setError(''); setToken('') }}
                className="w-full text-[rgba(0,255,100,0.3)] font-[family-name:var(--font-mono)] text-[10px] tracking-[2px] uppercase hover:text-[rgba(0,255,100,0.6)] transition-all"
              >
                ← voltar
              </button>
