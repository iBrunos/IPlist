...
function isProtectedIp(ip: string): string | null {
  ...
  if (PROTECTED_IPS.has(base)) return `${base} é um servidor DNS público protegido`
...
export default function IpsPage() {
  ...
  const [form, setForm] = useState({ address: '', cidr: '32', description: '', expiresAt: '' })
  ...
  const openCreate = () => {
    setEditingIp(null)
    setForm({ address: '', cidr: '32', description: '', expiresAt: '' })
    setFormError('')
    setShowModal(true)
  }

  const openEdit = (ip: Ip) => {
    const [addr, cidr] = ip.address.includes('/') ? ip.address.split('/') : [ip.address, '32']
    setEditingIp(ip)
    setForm({ address: addr, cidr, description: ip.description || '', expiresAt: ip.expiresAt ? ip.expiresAt.split('T')[0] : '' })
    setFormError('')
    setShowModal(true)
  }

  const handleAddressChange = (value: string) => {
    const formatted = formatIpInput(value)
    setForm(f => ({ ...f, address: formatted }))
    const protection = isProtectedIp(formatted)
  ...
  const handleSave = async () => {
    ...
    setFormError('')
    try {
      const address = `${form.address}/${form.cidr}`
      const expiresAt = form.expiresAt || (() => {
        const d = new Date(); d.setDate(d.getDate() + 30); return d.toISOString().split('T')[0]
      })()
      if (editingIp) {
        await api.put(`/ips/${editingIp.id}`, { ...form, address, expiresAt })
      } else {
        await api.post('/ips', { ...form, address, expiresAt })
      }
      setShowModal(false)
      fetchIps()
    } catch (err: any) {
  ...
  const handleApprove = (id: string) => {
    setConfirm({
      message: 'Deseja aprovar este IP para o feed?',
      onConfirm: async () => {
        setConfirm(null)
        try { await api.put(`/ips/${id}/approve`); fetchIps() }
  ...
  const handleDelete = (id: string, address: string) => {
    setConfirm({
      message: `Deseja remover o IP ${address}?`,
      onConfirm: async () => {
        setConfirm(null)
        try { await api.delete(`/ips/${id}`); fetchIps() }
    ...
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-[family-name:var(--font-mono)] text-[11px] text-[rgba(0,255,100,0.6)] tracking-[4px] uppercase">Gerenciamento de IPs</h1>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[rgba(0,255,100,0.08)]">
                  {['Endereço IP/CIDR', 'Descrição', 'Status', 'Expira em', 'Criado por', 'Criado em', 'Ações'].map(h => (
                ...
                {ips.map(ip => (
                  <tr key={ip.id} className="border-b border-[rgba(0,255,100,0.04)] hover:bg-[rgba(0,255,100,0.02)] transition-all">
                    ...
                    <td className="px-4 py-3">
                      <span className={`font-[family-name:var(--font-mono)] text-[9px] tracking-[2px] uppercase px-2 py-1 border ${statusColor[ip.status] || 'text-white/40 border-white/10'}`}>
                        {ip.status === 'approved' ? 'Aprovado' : 'Pendente'}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-[family-name:var(--font-mono)] text-[11px] text-[rgba(255,255,255,0.4)]">{ip.expiresAt ? new Date(ip.expiresAt).toLocaleDateString('pt-BR') : '—'}</td>
                    <td className="px-4 py-3 font-[family-name:var(--font-mono)] text-[11px] text-[rgba(0,255,100,0.5)]">{ip.createdBy?.username || '—'}</td>
                    <td className="px-4 py-3 font-[family-name:var(--font-mono)] text-[11px] text-[rgba(255,255,255,0.3)]">{new Date(ip.createdAt).toLocaleDateString('pt-BR')}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        {canApprove && ip.status === 'pending' && (
                          <button onClick={() => handleApprove(ip.id)} className="font-[family-name:var(--font-mono)] text-[9px] tracking-[2px] px-2 py-1 border border-[rgba(0,255,100,0.3)] text-[rgba(0,255,100,0.7)] hover:bg-[rgba(0,255,100,0.08)] transition-all">APROVAR</button>
                        )}
                        <button onClick={() => openEdit(ip)} className="font-[family-name:var(--font-mono)] text-[9px] tracking-[2px] px-2 py-1 border border-[rgba(255,255,255,0.1)] text-[rgba(255,255,255,0.4)] hover:border-[rgba(0,255,100,0.3)] hover:text-[rgba(0,255,100,0.7)] transition-all">EDITAR</button>
                        <button onClick={() => handleDelete(ip.id, ip.address)} className="font-[family-name:var(--font-mono)] text-[9px] tracking-[2px] px-2 py-1 border border-[rgba(255,50,50,0.2)] text-[rgba(255,100,100,0.5)] hover:bg-[rgba(255,50,50,0.05)] hover:border-[rgba(255,50,50,0.4)] transition-all">APAGAR</button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {ips.map(ip => (
              <div key={ip.id} className="relative bg-[rgba(255,255,255,0.02)] border border-[rgba(0,255,100,0.1)] p-4">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-[family-name:var(--font-mono)] text-sm text-white tracking-wider">{ip.address}</p>
                    <p className="font-[family-name:var(--font-mono)] text-[10px] text-[rgba(255,255,255,0.3)] mt-0.5">{ip.description || '—'}</p>
                  </div>
                  <span className={`font-[family-name:var(--font-mono)] text-[9px] tracking-[2px] uppercase px-2 py-1 border ${statusColor[ip.status] || 'text-white/40 border-white/10'}`}>
                    {ip.status === 'approved' ? 'Aprovado' : 'Pendente'}
                  </span>
                ...
                <div className="flex items-center gap-4 mb-3">
                  <div>
                    ...
                    <p className="font-[family-name:var(--font-mono)] text-[10px] text-[rgba(255,255,255,0.4)]">{ip.expiresAt ? new Date(ip.expiresAt).toLocaleDateString('pt-BR') : '—'}</p>
                  </div>
                  <div>
                    <p className="font-[family-name:var(--font-mono)] text-[8px] text-[rgba(0,255,100,0.4)] tracking-[2px] uppercase">Criado por</p>
                    <p className="font-[family-name:var(--font-mono)] text-[10px] text-[rgba(0,255,100,0.6)]">{ip.createdBy?.username || '—'}</p>
                  </div>
                  <div>
                    <p className="font-[family-name:var(--font-mono)] text-[8px] text-[rgba(0,255,100,0.4)] tracking-[2px] uppercase">Data</p>
                    <p className="font-[family-name:var(--font-mono)] text-[10px] text-[rgba(255,255,255,0.3)]">{new Date(ip.createdAt).toLocaleDateString('pt-BR')}</p>
                  </div>
                </div>
                <div className="flex gap-2 pt-3 border-t border-[rgba(0,255,100,0.06)]">
                  {canApprove && ip.status === 'pending' && (
                    <button onClick={() => handleApprove(ip.id)} className="flex-1 font-[family-name:var(--font-mono)] text-[9px] tracking-[2px] py-1.5 border border-[rgba(0,255,100,0.3)] text-[rgba(0,255,100,0.7)] hover:bg-[rgba(0,255,100,0.08)] transition-all">APROVAR</button>
                  )}
                  <button onClick={() => openEdit(ip)} className="flex-1 font-[family-name:var(--font-mono)] text-[9px] tracking-[2px] py-1.5 border border-[rgba(255,255,255,0.1)] text-[rgba(255,255,255,0.4)] hover:border-[rgba(0,255,100,0.3)] hover:text-[rgba(0,255,100,0.7)] transition-all">EDITAR</button>
                  <button onClick={() => handleDelete(ip.id, ip.address)} className="flex-1 font-[family-name:var(--font-mono)] text-[9px] tracking-[2px] py-1.5 border border-[rgba(255,50,50,0.2)] text-[rgba(255,100,100,0.5)] hover:bg-[rgba(255,50,50,0.05)] transition-all">APAGAR</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowModal(false)}>
          <div className="relative bg-[#030a0e] border border-[rgba(0,255,100,0.2)] w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            ...
            <h2 className="font-[family-name:var(--font-mono)] text-[11px] text-[rgba(0,255,100,0.7)] tracking-[4px] uppercase mb-6">
              {editingIp ? 'Editar IP' : 'Adicionar IP'}
            </h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <span className="font-[family-name:var(--font-mono)] text-[rgba(0,255,100,0.4)] flex items-center px-1">/</span>
                  <select value={form.cidr} onChange={e => setForm({ ...form, cidr: e.target.value })} disabled={!!editingIp}
                    className="w-20 bg-black/40 border border-[rgba(0,255,100,0.15)] px-2 py-2.5 text-white font-[family-name:var(--font-mono)] text-sm outline-none focus:border-[rgba(0,255,100,0.5)] transition-all disabled:opacity-40 [color-scheme:dark]">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={handleSave} disabled={saving || !form.address || isFormBlocked}
