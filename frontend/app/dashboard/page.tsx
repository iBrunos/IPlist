...
// Hashes conhecidas de arquivos legítimos/protegidos que não devem ser bloqueadas
const PROTECTED_HASHES = new Set([
  // Windows System Files (MD5)
  'd41d8cd98f00b204e9800998ecf8427e', // MD5 empty file
  'da39a3ee5e6b4b0d3255bfef95601890', // SHA1 empty file
  'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855', // SHA256 empty file
])

const PROTECTED_HASH_PREFIXES = [
  '000000000000000000000000', // Hashes zeradas
]
...
function isProtectedHash(value: string, type: string): string | null {
  ...
  // Bloquear hashes com todos os caracteres iguais (ex: aaaaaa...)
...
export default function HashesPage() {
  ...
  const filtered = hashes.filter(h =>
    ...
    (h.createdBy?.username || '').toLowerCase().includes(search.toLowerCase())
  )
  ...
  const handleValueChange = (value: string) => {
    ...
    if (protection) {
      setFormError(`⛔ ${protection}`)
    } else if (clean.length > 0 && clean.length === hashLengths[form.type]) {
      setFormError('')
    } else {
      setFormError('')
    }
